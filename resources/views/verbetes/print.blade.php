  ...
  <div class="sheet-outer A4">
    <section class="sheet padding-5mm">
      ...
      <div class="sheet-content">
        ...
        {{-- Verbete Completo --}}
        @if($verbete->author)<div class="fw-bold">{{$verbete->author}}</div>@endif
        <div class="text-justify mb-5">
          <p>
            <span class="fst-italic text-danger">{{$verbete->title}}. </span>
            <span>{{$verbete->mentions}}. </span>
            <span>{{$verbete->place.': '.$verbete->printer.', '.$verbete->date.'.'}}</span>
          </p>
        ...
        {{-- Relatório de Vendas em Leilão --}}

        <x-infoCard.infoTable>

          <x-slot:tableHeader>
            <th class="text-xsmall text-center">Leilão</th>
            <th class="text-xsmall text-center">Lote</th>
            <th class="text-xsmall text-end">Base</th>
            <th class="text-xsmall text-end">Martelo</th>
          </x-slot:tableHeader>
          <x-slot:tableBody>
            @foreach($itemsLeilao as $item)
            <tr>
              <td class="text-xsmall text-center">{{$item->leilao_id}}</td>
              <td class="text-xsmall text-center">{{$item->leilao_lote}}</td>
              <td class="text-xsmall text-end">{{$item->start_price}} €</td>
              <td class="text-xsmall text-end">{{$item->price}} €</td>
              <td></td>
            </tr>
            @endforeach
          </x-slot:tableBody>
